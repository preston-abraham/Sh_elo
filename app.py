...
def load_player_elos():
    ...
    try:
        worksheet = get_worksheet("Players")
        if not worksheet:
            return {}
...
def save_player_elos(player_elos):
    ...
    try:
        ...
        for player, elo in player_elos.items():
            data_to_upload.append([player, elo, datetime.now().strftime('%Y-%m-%d %H:%M:%S')])
...
def save_match_history(match_data):
    ...
    try:
        worksheet = get_worksheet("Match_History")
        if not worksheet:
            return False
        ...
        # Prepare match data for upload
        row_data = [
            match_data['date'],
            ', '.join(match_data['liberal_team']),
            ', '.join(match_data['fascist_team']),
            match_data['hitler'],
            match_data['winning_team'],
            match_data['game_end_condition'],
            match_data.get('shooter', ''),
            match_data.get('shot_player', ''),
        ...
        worksheet.update(f'A{next_row}:K{next_row}', [row_data])
        return True
